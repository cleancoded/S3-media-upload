 ...
 * ADMIN OPTIONS
 */
function CleancodedAdminMenu() {
    add_options_page('S3 Media Storage', 'S3 Media Storage', 'manage_options', 'CleancodedAdminMenu', 'CleancodedAdminContent');
}

add_action('admin_menu', 'CleancodedAdminMenu');

function CleancodedAdminContent() {
    if (isset($_POST['submit'])) {
        ...
        if (!empty($errors)) {
            ...
            ?>
            <div class="error"><p><strong><?php _e($msg, 'Cleancoded' ); ?></strong></p></div>
            <?php
        } else {
            // No errors!
            $settings = array(
            ...
            $ret = update_option('Cleancoded_settings', $settings);

            ?>
            <div class="updated"><p><strong><?php _e('Settings Saved!', 'Cleancoded' ); ?></strong></p></div>
            <?php
        }
    }
    ...
    if (isset($_POST['move_files']) || isset($_POST['copy_files'])) {
        ...
        if (isset($_POST['selected']) && is_array($_POST['selected'])) {
            ...
            foreach ($_POST['selected'] as $id) {
                $ret = Cleancoded::updateAttachmentMetadata(array('Cleancoded_move' => $move), $id);
                if ($ret) {
                    $success_count++;
                } else {
                    $error_count++;
            ...
            ?>
            <div class="updated"><p><strong><?php _e(number_format($success_count) . ' File(s) '.$label.'!', 'Cleancoded' ); ?></strong></p></div>
            <?php
            if ($error_count > 0) {
                ?>
                <div class="error"><p><strong><?php _e(number_format($error_count) . ' File(s) Could Not Be '.$label.'!', 'Cleancoded' ); ?></strong></p></div>
                <?php
            }
        }
    }
    ...
    $settings = json_decode(get_option('Cleancoded_settings'), true);
...
<div class="wrap">
	...
	<div id="poststuff">
		...
		<h3><?php _e('Settings'); ?></h3>
            <div class="inside">
            <form id="Cleancoded-config" method="post" action="" enctype="multipart/form-data">

                <table class="form-table">
                    <tbody>
                        <tr>
                            <th><label for="key"><?php _e("S3 Bucket Name:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("S3 Bucket Path:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Delete Local Files:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Delete from S3:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Expires:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("CloudFront Domain Name:", 'Cleancoded' ); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th><label for="key"><?php _e("Protocol:", 'Cleancoded' ); ?></label></th>
                            <td>
    ...
    <div id="poststuff">
		...
		<h3><?php _e('Library Files'); ?></h3>
            <div class="inside">
            <form id="Cleancoded-transfer" method="post" action="" enctype="multipart/form-data">
                ...
                global $wpdb;
                $page = isset($_GET['Cleancoded_page']) ? (int) $_GET['Cleancoded_page'] : 1;
                $limit = 100;
                $offset = ($limit * $page) - $limit;

                $sql = "SELECT COUNT(1) as count
                        FROM {$wpdb->posts}
                        LEFT JOIN {$wpdb->postmeta} ON {$wpdb->postmeta}.post_id = {$wpdb->posts}.ID AND {$wpdb->postmeta}.meta_key = 'Cleancoded_file'
                        WHERE 1=1
                ...
                $page_action = Cleancoded::paginationUrl();
                ...
                <table class="wp-list-table widefat">
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="5" scope="row"><p style="float:left"><?php echo Cleancoded::pagination($total, $limit, $page, $page_action);?></p>&nbsp;<p style="float:right">Page <?php echo number_format($page);?> of <?php echo number_format($pages);?> Total: <?php echo number_format($total);?> Files</p></th>
                        </tr>
                    </tfoot>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php $s3_url = Cleancoded::attachmentUrl($file->guid, $file->ID); echo $s3_url == $file->guid ? '' : '<a href="'.$s3_url.'" target="_blank">'.$s3_url.'</a>';?></td>
