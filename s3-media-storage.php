<?php
/*
Plugin Name: Cleancoded S3 Media Storage
...
if ( !function_exists( 'add_action' ) ) {
	echo 'Cleancoded S3 Media Storage WordPress Plugin';
	exit;
}

define('Cleancoded_PLUGIN_VERSION', '1.0.3');
define('Cleancoded_PLUGIN_URL', plugin_dir_url( __FILE__ ));

register_activation_hook(__FILE__, array('Cleancoded', 'install'));
register_uninstall_hook(__FILE__, array('Cleancoded', 'uninstall'));
...
class Cleancoded {
    ...
    {
        if (!self::$settings) {
            self::$settings = json_decode(get_option('Cleancoded_settings'), true);
        }
    ...
    public static function attachmentUrl($url, $post_id)
    {
        ...
        $bucket = isset($custom_fields['Cleancoded_bucket']) ? $custom_fields['Cleancoded_bucket'][0] : null;
        $bucket_path = isset($custom_fields['Cleancoded_bucket_path']) ? $custom_fields['Cleancoded_bucket_path'][0] : null;
        ...
        // $file = isset($custom_fields['Cleancoded_file']) ? $custom_fields['Cleancoded_file'][0] : null;
        $cloudfront = isset($custom_fields['Cleancoded_cloudfront']) ? $custom_fields['Cleancoded_cloudfront'][0] : null;
    ...
    public static function deleteAttachment($url)
    {
        ...
        if (isset($settings['valid']) && (int) $settings['valid']) {
            $s3 = Cleancoded_Transfer::getTransferClass($settings);
    ...
    public static function paginationUrl()
    {
        ...
        unset($out['Cleancoded_page']);
        $out = http_build_query($out);
        if ($out) {
            return '?' . $out . '&Cleancoded_page=';
        }
        return '?Cleancoded_page=';
    }
...
class Cleancoded_Transfer {
    ...
    public static function getTransferClass($settings)
    {
        ...
        if ($transfer_class == 's3class') {
            self::$transfer_class = new Cleancoded_Transfer_Adapter_S3Class($settings);
        } elseif ($transfer_class == 's3cmd') {
            self::$transfer_class = new Cleancoded_Transfer_Adapter_S3Cmd($settings);
        } elseif ($transfer_class == 'awscli') {
            self::$transfer_class = new Cleancoded_Transfer_Adapter_AwsCli($settings);
        } elseif ($transfer_class == 'background') {
            self::$transfer_class = new Cleancoded_Transfer_Adapter_Background($settings);
        } else {
            throw new Cleancoded_Transfer_Exception('Invalid Transfer Class.');
            return;
        }
...
class Cleancoded_Transfer_Exception extends Exception {
...
abstract class Cleancoded_Transfer_Adapter {
...
class Cleancoded_Transfer_Adapter_S3Class {
    ...
    public function upload($attachment_path, $s3_path, $attachment_id = false, $data = null)
    {
        ...
        try {
            ...
            if ($attachment_id) {
                ...
                update_post_meta($attachment_id, "Cleancoded_bucket", $this->settings['s3_bucket']);
                update_post_meta($attachment_id, "Cleancoded_bucket_path", $this->settings['s3_bucket_path']);
                update_post_meta($attachment_id, "Cleancoded_file", $s3_path);
                update_post_meta($attachment_id, "Cleancoded_cloudfront", $this->settings['s3_cloudfront']);
                if ((isset($data['Cleancoded_move']) && $data['Cleancoded_move']) || (isset($this->settings['s3_delete_local']) && $this->settings['s3_delete_local'])) {
                    @unlink($attachment_path);
                }

                // If we are copy or moving we need to grab any thumbnails as well.
                if (isset($data['Cleancoded_move'])) {
                    ...
                    if (isset($c['sizes']) && is_array($c['sizes'])) {
                        foreach ($c['sizes'] as $size) {
                            ...
                            if (isset($size['file']) && strpos($size['file'], '-') && strpos($size['file'], 'x')) {
                                ...
                                Cleancoded::imageMakeIntermediateSize($new_attachment_path);
                            }
                        }
                    }
                }
...
class Cleancoded_Transfer_Adapter_S3Cmd {

}


class Cleancoded_Transfer_Adapter_AwsCli {

}


class Cleancoded_Transfer_Adapter_Background {
 ...
 *
 */
class Cleancoded_Transfer_Adapter_Background_Response {
...
add_filter('wp_update_attachment_metadata', array('Cleancoded', 'updateAttachmentMetadata'), 9, 2);
// Handle thumbs that are created for that image
add_filter('image_make_intermediate_size', array('Cleancoded', 'imageMakeIntermediateSize'));
// Handle when image urls are requested.
add_action("wp_get_attachment_url", array('Cleancoded', 'attachmentUrl'), 9, 2);
add_action("wp_get_attachment_thumb_url", array('Cleancoded', 'attachmentUrl'), 9, 2);
// Handle when images are deleted.
add_action("wp_delete_file", array('Cleancoded', 'deleteAttachment'));
